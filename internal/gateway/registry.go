...
func (r *Registry) Register(g State) error {
	if g.ID == "" { return fmt.Errorf("gateway id is required") }
	if g.Socket == "" { return fmt.Errorf("gateway %q socket is required", g.ID) }
	if _, exists := r.gateways[g.ID]; exists { return fmt.Errorf("gateway %q already registered", g.ID) }
	r.gateways[g.ID] = g
	return nil
}
...
func (r *Registry) IDs() []string {
	...
	for id := range r.gateways { ids = append(ids, id) }
	sort.Strings(ids)
	return ids
}
