...
func Discover(ctx context.Context, id string, cfg config.Gateway) (State, error) {
	if !cfg.Enabled {
		return State{}, fmt.Errorf("gateway %q is disabled", id)
	}
	d := net.Dialer{Timeout: 2 * time.Second}
	conn, err := d.DialContext(ctx, "unix", cfg.Socket)
	if err != nil {
		return State{}, fmt.Errorf("gateway %q unavailable: %w", id, err)
	}
	defer conn.Close()
	...
	if err := json.NewDecoder(conn).Decode(&h); err != nil {
		return State{}, fmt.Errorf("gateway %q invalid hello: %w", id, err)
	}
	if h.ID != id {
		return State{}, fmt.Errorf("gateway identity mismatch: configured %q, reported %q", id, h.ID)
	}
	if !h.Healthy {
		return State{}, fmt.Errorf("gateway %q is unhealthy", id)
	}
