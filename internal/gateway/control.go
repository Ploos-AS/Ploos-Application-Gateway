...
func (s ControlServer) Serve() (*ControlListener, error) {
	if s.ID == "" || s.Socket == "" {
		return nil, fmt.Errorf("gateway control id and socket are required")
	}
	...
	if fi, err := os.Lstat(s.Socket); err == nil {
		if fi.Mode()&os.ModeSocket == 0 {
			return nil, fmt.Errorf("refusing to replace non-socket control path %q", s.Socket)
		}
...
func (s ControlServer) handle(c net.Conn) {
	...
	if err := json.NewDecoder(c).Decode(&req); err != nil || req["op"] != "hello" {
		return
	}
	healthy := true
	if s.Healthy != nil {
