...
func TestBurstLimit(t *testing.T) {
	...
	if !l.Allow(a)||!l.Allow(a) { t.Fatal("initial burst rejected") }
	if l.Allow(a) { t.Fatal("burst limit not enforced") }
}

func TestClientsIndependent(t *testing.T) {
	l:=New(1,1,0)
	a:=addr("192.0.2.1:1"); b:=addr("192.0.2.2:1")
	if !l.Allow(a)||!l.Allow(b) { t.Fatal("clients must have independent buckets") }
}

func TestRefill(t *testing.T) {
	l:=New(1,1,0); now:=time.Unix(0,0); l.now=func() time.Time{return now}
	a:=addr("192.0.2.1:1")
	if !l.Allow(a)||l.Allow(a) { t.Fatal("unexpected initial bucket behavior") }
	now=now.Add(time.Second)
	if !l.Allow(a) { t.Fatal("token did not refill") }
}

func TestConcurrency(t *testing.T) {
	l:=New(1,1,1); a:=addr("192.0.2.1:1")
	if !l.Acquire(a) { t.Fatal("first acquire rejected") }
	if l.Acquire(a) { t.Fatal("concurrency limit not enforced") }
	l.Release(a)
