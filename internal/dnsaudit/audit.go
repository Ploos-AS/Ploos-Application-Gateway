...
type Event struct { Time string `json:"time"`; Decision string `json:"decision"`; Transport string `json:"transport"`; QType uint16 `json:"qtype,omitempty"`; Reason string `json:"reason,omitempty"` }
...
func (l *Logger) Log(decision, transport string, qtype uint16, reason string) {
 ...
 l.mu.Lock(); defer l.mu.Unlock(); _=json.NewEncoder(l.w).Encode(e)
}
