...
func ValidateResponse(query, response []byte) error {
	if err := ValidateQuery(query); err != nil {
		return fmt.Errorf("invalid original query: %w", err)
	}
	if len(response) < 12 {
		return fmt.Errorf("DNS response shorter than header")
	}
	...
	if (qflags>>11)&0x0f != (rflags>>11)&0x0f {
		return fmt.Errorf("DNS opcode mismatch")
	}
	...
	if binary.BigEndian.Uint16(response[4:6]) != 1 {
		return fmt.Errorf("DNS response must contain exactly one question")
	}
	...
	if !strings.EqualFold(qq.Name, rq.Name) {
		return fmt.Errorf("DNS question name mismatch")
	}
	if qq.Type != rq.Type {
		return fmt.Errorf("DNS question type mismatch")
	}
	if qq.Class != rq.Class {
		return fmt.Errorf("DNS question class mismatch")
	}
	return nil
}
