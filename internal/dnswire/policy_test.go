...
func TestAnswerSectionRejectedInQuery(t *testing.T) {
	q:=query([]byte{1,'a',0}); q[7]=1
	if err:=ValidateQueryPolicy(q,DefaultPolicy()); err==nil { t.Fatal("answer section accepted in query") }
}

func TestAuthoritySectionRejectedInQuery(t *testing.T) {
	q:=query([]byte{1,'a',0}); q[9]=1
	if err:=ValidateQueryPolicy(q,DefaultPolicy()); err==nil { t.Fatal("authority section accepted in query") }
}

func TestTrailingDataRejectedWithoutAdditional(t *testing.T) {
	q:=append(query([]byte{1,'a',0}),0)
	if err:=ValidateQueryPolicy(q,DefaultPolicy()); err==nil { t.Fatal("trailing data accepted") }
}

func TestEDNSVersionOneRejected(t *testing.T) {
	q:=query([]byte{1,'a',0}); q[11]=1
	q=append(q,0,0,41,0x04,0xd0,0,1,0,0,0,0)
	if err:=ValidateQueryPolicy(q,DefaultPolicy()); err==nil { t.Fatal("EDNS version 1 accepted") }
}

func TestEDNSReservedFlagsRejected(t *testing.T) {
	q:=query([]byte{1,'a',0}); q[11]=1
	q=append(q,0,0,41,0x04,0xd0,0,0,0,1,0,0)
	if err:=ValidateQueryPolicy(q,DefaultPolicy()); err==nil { t.Fatal("reserved EDNS flags accepted") }
}
