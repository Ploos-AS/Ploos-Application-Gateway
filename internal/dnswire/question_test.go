...
func TestParseQuestion(t *testing.T) {
	...
	if q.Name!="www.example.com" || q.Type!=1 || q.Class!=1 { t.Fatalf("unexpected question: %+v",q) }
}

func TestCompressionLoopRejected(t *testing.T) {
	m:=make([]byte,12); m[5]=1
	m=append(m,0xc0,0x0c,0,1,0,1)
	if _,err:=ParseQuestion(m); err==nil { t.Fatal("compression loop accepted") }
}

func TestPointerOutOfBoundsRejected(t *testing.T) {
	m:=make([]byte,12); m[5]=1
	m=append(m,0xc0,0xff,0,1,0,1)
	if _,err:=ParseQuestion(m); err==nil { t.Fatal("out-of-bounds pointer accepted") }
}

func TestTruncatedLabelRejected(t *testing.T) {
	m:=make([]byte,12); m[5]=1
	m=append(m,5,'a','b')
	if _,err:=ParseQuestion(m); err==nil { t.Fatal("truncated label accepted") }
}
