...
func TestRenderIsDefaultDeny(t *testing.T) {
	...
	if !strings.Contains(out, "policy drop;") { t.Fatal("forward chain must default to drop") }
	if strings.Contains(out, " accept") { t.Fatal("M1 renderer must not create forwarding accept rules") }
}
...
func TestUnsafeInterfaceRejected(t *testing.T) {
	...
	if _, err := Render(c); err == nil { t.Fatal("unsafe interface name must be rejected") }
}
