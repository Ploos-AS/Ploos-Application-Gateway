...
func TestDefaultDeny(t *testing.T) {
	...
	if d := Evaluate(c, "lan", "wan", "smtp"); d.Allowed { t.Fatal("unmatched traffic must be denied") }
}
...
func TestProxyRule(t *testing.T) {
	...
	if !d.Allowed || d.Gateway != "pag-dns" { t.Fatalf("unexpected decision: %+v", d) }
}
...
func TestMissingGatewayFailsClosed(t *testing.T) {
	...
	if d := Evaluate(c, "lan", "wan", "smtp"); d.Allowed { t.Fatal("missing gateway must fail closed") }
}
